...
const TopKBooks = () => {
    ...
    async function showClick(event) {
        ...
        let data = []
        try {
            let response = await fetch(`http://127.0.0.1:5000/api/top/book?k=${Knum}`, {
                method: 'Get'
            });
        ...
        setMessage(`Top ${arr_length} Books:`);
        render(data);
    }
    ...
    const render = (data) => {
        ...
        g.append('g').call(d3.axisLeft(yScale)).style("font", "6px times");
        g.append('g').call(d3.axisBottom(xScale)).attr('transform', `translate(0, ${innerHeight})`).style("font", "12px");
